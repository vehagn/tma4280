...
void printToFile(double**u, int n, int m, int rank){
    /*MPI_File fh;
    MPI_File_open(MPI_COMM_WORLD,"MPIfile.dat",MPI_MODE_WRONLY|MPI_MODE_CREATE,MPI_INFO_NULL,&fh);
    MPI_Offset mysize = u.m*u.local_m;

    MPI_File_seek(fh,rank*mysize*sizeof(double),MPI_SEEK_SET);
    //for (int i = 0; i < u.local_m; i++){
		//for (int j = 0; j < u.m; j++){
            MPI_File_write(fh,u.data,mysize,MPI_DOUBLE,MPI_STATUS_IGNORE);
		//}
		MPI_File_write(fh,"\n",mysize,MPI_CHAR,MPI_STATUS_IGNORE);
    //}
